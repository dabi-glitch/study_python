...
character_x_pos = (screen_width / 2) - (character_width / 2) # 화면 가로의 절반 크기에 해당하는 곳에 위치한다. 
character_y_pos = screen_height - character_height # 화면 세로 크기 가장 아래에 해당하는 곳에 위치한다. 
...
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) # 화면 가로의 절반 크기에 해당하는 곳에 위치한다. 
enemy_y_pos = (screen_height /2) - (enemy_height / 2) # 화면 세로 크기 가장 아래에 해당하는 곳에 위치한다. 
...
while running:
    dt = clock.tick(60) # 게임화면의 초당 프레임 수를 설정한다, frame을 너무 낮게 설정하면 버퍼링
